...
function CompileAndCopyFsScan {
	...
	pushd $CODE_DIR/src/fsScanLinuxUnix/fsscan/;
	make clean;
	make;
	popd;
...
function RunMultipleScansWithFlags {
	...
	echo "Test";
	echo "$ROOT_TEST_SCAN_LOCATION" -dtl "${DTL_FILE}_test1.dtl" -tag "$SCAN_TAG" > "$OUTPUT_DIR/test_outputs/JIRA_FSMA-${JIRA_NUMBER}_TEST_1_FSSCAN_OUTPUT.txt";
		
	$FSSCAN_APP "$ROOT_TEST_SCAN_LOCATION" -dtl "${DTL_FILE}_test1.dtl" -tag "$SCAN_TAG" > "$OUTPUT_DIR/test_outputs/JIRA_FSMA-${JIRA_NUMBER}_TEST_1_FSSCAN_OUTPUT.txt";

	echo "Starting second test scan";
	$FSSCAN_APP -cfg "$SCAN_CONFIGS_DIR/test3.cfg" -dtl "${DTL_FILE}_test3.dtl" -tag "$SCAN_TAG" > "$OUTPUT_DIR/test_outputs/JIRA_FSMA-${JIRA_NUMBER}_TEST_3_FSSCAN_OUTPUT.txt";

	echo "Starting third test scan";
	$FSSCAN_APP -cfg "$SCAN_CONFIGS_DIR/test4.cfg" -dtl "${DTL_FILE}_test4.dtl" -tag "$SCAN_TAG" > "$OUTPUT_DIR/test_outputs/JIRA_FSMA-${JIRA_NUMBER}_TEST_4_FSSCAN_OUTPUT.txt";

	echo "Starting fourth test scan";
	$FSSCAN_APP -cfg "$SCAN_CONFIGS_DIR/test5.cfg" -dtl "${DTL_FILE}_test5.dtl" -tag "$SCAN_TAG" > "$OUTPUT_DIR/test_outputs/JIRA_FSMA-${JIRA_NUMBER}_TEST_5_FSSCAN_OUTPUT.txt";

	CreateTestFiles

	echo "Starting sixth test scan";
	$FSSCAN_APP "$ROOT_TEST_SCAN_LOCATION" -dtl "${DTL_FILE}_test2.dtl" -tag "$SCAN_TAG" > "$OUTPUT_DIR/test_outputs/JIRA_FSMA-${JIRA_NUMBER}_TEST_2_FSSCAN_OUTPUT.txt";

	zip -r $OUTPUT_DIR/test_outputs/JIRA_FSMA-${JIRA_NUMBER}_DTLS.zip $OUTPUT_DIR/$DTL_FILE_DIR/*;
