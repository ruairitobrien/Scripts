...
# Where text file outputs for scan tests will go
TEST_OUTPUT_DIR=$OUTPUT_DIR/test_outputs

# Location of test files to be scanned
ROOT_TEST_SCAN_LOCATION=$DIR/FSMA_TARGET_SCAN_FOLDER
...
# Three forms of the pushd command for Korn Shell:
...
# popd function implementation for Korn Shell
function popd
{
   sd=${#dir_stack[*]}
   if [ $sd -gt 0 ] ; then
...
# Create any directories required for testing
...
function compileAndCopyFsScan 
{
	...
	pushd $CODE_DIR/fsScanLinuxUnix/fsscan/;
	make clean;
	make;
	popd 
	cp $CODE_DIR/fsScanLinuxUnix/fsscan/fsscan $FSSCAN_APP;
...
{
	echo "Creating test files at: $GENERATED_TEST_FILES"; 
...
# Checks the fsScan application can be run. Creates an output folder for DTL files. Runs multiple scans based
